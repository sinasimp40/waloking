...
7. **Transactional extract (Task #17):** `extractZip` in `electron/updater.js` and `server/electron/updater.js` is per-file transactional. Existing targets are renamed into `.ota-pending/.ota-bak/<entry>` before overwrite; locked files (asar, ffmpeg.dll, icudtl.dat, v8 snapshot) fail at the rename and stay untouched on disk. Successful entries are pushed onto a caller-owned `successfulEntries` out-array. On any failure (per-entry write failure, index.html sanity check, outer-catch when extractZip throws), the apply path calls `rollbackExtract` which deletes new files and renames backups back into place. Eliminates the black/violet screen caused by OLD launcher + MIX of OLD locked asar + NEW unlocked chrome paks. Safety rules: `rollbackExtract` never unlinks a replacement without a `backupPath`; apply refuses to extract if `.ota-bak` cannot be created; leftover `.ota-bak` from a killed previous attempt is recovered in place before being wiped. Diagnostics report `rolledBack.{restored, removed, skipped}` on the FAILED marker. Covered by `tests/test-rebrand-update.js` and `tests/test-server-rebrand.js`.
