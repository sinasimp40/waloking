...
- **OS-chord blocking — INTENTIONALLY ABSENT:** Two approaches were tried and removed. (A) Focus-snap on blur — Electron only fires blur AFTER focus has moved, so the taskbar visibly flashes before snapping back. (B) Native `WH_KEYBOARD_LL` hook + `Shell_TrayWnd` SW_HIDE — the hook is installed in the launcher process, but Windows 11's Alt+Tab switcher runs in a separate DWM/explorer process that consumes the chord before our hook sees it under heavy spam, so the taskbar still leaks through. **Conclusion: in-process userland code cannot reliably block Alt+Tab on Windows 11.** Real Alt+Tab lockdown for production cafe deployments requires one of: (1) killing `explorer.exe` for the duration of the launcher session and respawning it on exit; (2) Windows built-in **Assigned Access** kiosk mode (Settings → Accounts → Other users → Set up a kiosk), which is Microsoft's sanctioned single-app lockdown; (3) third-party shell replacement like SiteKiosk. None of these are implemented yet — operators currently rely on the construction-time fullscreen + the cafe's physical environment to discourage tampering.
