...
8. **Out-of-process Windows applier (Task #18 — fixes the field-reported "OLD launcher keeps running, payload never extracts" bug):** On Windows the in-process path can never overwrite `app.asar` (the running Electron process holds the lock), so `applyPendingUpdateOnStartup` takes a different path. (a) Acquire `.ota-pending/.apply.lock` via atomic `O_CREAT|O_EXCL` — if held and <5 min old, skip; >5 min stale locks get taken over. (b) Validate `manifest.exeName` against `^[A-Za-z0-9._-]+\.exe$` whitelist before doing anything. (c) Extract the payload to `.ota-pending/staged/` (clean dir, no locks). (d) **Pre-stage all metadata files into `staged/` from Node**, where `JSON.stringify` handles all escaping safely: `.ota-cleanup.json` (deletes OLD exe on next launch), `.ota-current-exe.json` (sidecar so a manually-launched OLD exe self-handoffs), and a **version-bumped clone of the OLD `ota-config.json`** that preserves customer-specific fields (channel, customerId, updateServer) and only changes `version`. (e) Write `apply.cmd` to `%TEMP%` and spawn `cmd.exe /c apply.cmd <parentPid> <pendingDir> <installDir> <newExe>` detached + windowsHide. (f) Immediately `app.exit(0)` so the OS releases the asar handle. The cmd applier then polls `tasklist` for parent exit (up to 30s), waits 3s grace for handle release, runs `robocopy /E /MOVE /R:5 /W:2` (~10s extra retry budget for transient locks), launches the NEW exe, and self-deletes. **Crucially, the cmd script contains zero PowerShell** — every JSON write happens in Node before spawn, eliminating the string-interpolation injection surface that an earlier draft had via `manifest.version` / `manifest.exeName`. Args passed to cmd are limited to: process.pid (integer), filesystem paths the caller already controls, and the whitelisted exe basename. Test hooks (`_forceOutOfProcess`, `_spawnFn`, `_exitFn`, `_tmpDir`, `_cmdScript`) let the Linux test runner exercise the staging path, args contract, pre-staged JSON contents, lock semantics, and the exeName whitelist. Covered by `tests/test-rebrand-update.js` (111 assertions) and `tests/test-server-rebrand.js` (58 assertions).
