...
- **Taskbar leak after explorer respawn (UNRESOLVED):** A fresh explorer.exe rebuilds the taskbar by enumerating top-level windows using rules that don't exactly match a `(visible && !tool && (no_owner||appwindow) && has_title)` heuristic. A first attempt at a leak-guard (snapshot HWND+heuristic-taskbar-eligibility before kill → diff and hide after respawn) was tried and removed because the heuristic didn't match the real explorer logic and the diff was empty for the apps the user was seeing leak in (e.g. "Session"). A correct fix would need to query the actual taskbar button list — e.g. via UI Automation on `Shell_TrayWnd → ... → MSTaskListWClass`, or cross-process `TB_GETBUTTON` reads on explorer's task-list toolbar. Both are non-trivial. Workarounds: (a) accept the leak as a kiosk side-effect, (b) reverse to hiding `Shell_TrayWnd` instead of killing explorer (keeps tray + hidden apps intact, but weakens Alt+Tab lockdown), (c) ship a user-configurable allowlist/blocklist of window-title patterns and hide matches after respawn.
