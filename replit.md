...
- **Shell kill (explorer.exe lifecycle):** When kiosk turns ON, `killExplorer()` runs `taskkill /F /IM explorer.exe`, which terminates the Windows shell entirely — no taskbar, no Start menu, no system tray, no Alt+Tab visual switcher. The launcher becomes the only visible surface. When kiosk turns OFF (admin toggle, emergency chord, or app exit), `restoreExplorer()` respawns explorer detached via `cmd /c start explorer.exe` so it survives the launcher process. Tracked via `kioskState.explorerKilled` to prevent double-kill / orphan respawn. Restore is wired into `will-quit`, `process.on('uncaughtException')`, and `process.on('exit')` as belt-and-suspenders — if the launcher crashes or is force-killed via Task Manager (SIGKILL) the user must manually run `explorer.exe` from Task Manager to recover the shell. No-op on non-Windows.
- **Tray-icon re-registration after explorer respawn:** Windows broadcasts the `TaskbarCreated` window message when explorer starts, which tray-aware apps (Discord, Steam, Razer, OneDrive, antivirus, etc.) listen for to re-add their notification-area icons. In practice many apps miss the initial broadcast because they race against explorer's startup. `rebroadcastTaskbarCreated()` posts `HWND_BROADCAST` of the `RegisterWindowMessage("TaskbarCreated")` value via a hidden PowerShell `Add-Type` call (no native addon needed), fired at 1.5s and 4s after `restoreExplorer()`. This forces every running tray app to re-register so tray icons that vanished during kiosk come back. Uses `PostMessage` (non-blocking, asynchronous broadcast) instead of `SendMessage` so misbehaving recipients can't hang the launcher.
- **Taskbar-leak guard:** A fresh explorer.exe rebuilds the taskbar by enumerating every visible top-level window, so background apps that kept their main window hidden via `ShowWindow(SW_HIDE)` but left `WS_EX_APPWINDOW` set (e.g. "Session"-style helpers) end up wrongly added to the new taskbar. To suppress this without a name-blocklist, `snapshotTopLevelWindowsSync()` runs BEFORE `killExplorer()` and captures every existing HWND + its taskbar-eligibility (visible, no owner unless app-window, not tool-window, non-empty title). After `restoreExplorer()` finishes, `hideLeakedTaskbarWindows()` re-enumerates and calls `ShowWindow(SW_HIDE)` ONLY on HWNDs that (a) existed in the snapshot, (b) were NOT taskbar-eligible at snapshot time, and (c) ARE now. This guarantees we never touch windows of processes/windows created during or after kiosk (e.g. a game launched on the way out — its HWND wasn't in the snapshot so it's skipped). Runs detached via PowerShell `Start-Sleep` + EnumWindows P/Invoke so it survives Node exit when the launcher auto-closes on game launch in kiosk mode. Wired into `applyKiosk(false)` and the `will-quit`/`uncaughtException`/`process.on('exit')` paths via `fireExitLeakGuard()`. Two passes (2s and 5s after explorer respawn) catch both fast and slow taskbar rebuilds.
- **Why not in-process key blocking:** Two prior approaches were removed. (A) Focus-snap on blur — Electron only fires blur AFTER focus has moved, so the taskbar visibly flashes before snapping back. (B) Native `WH_KEYBOARD_LL` hook + `Shell_TrayWnd` SW_HIDE — the hook is installed in the launcher process, but Windows 11's Alt+Tab switcher runs in DWM/explorer and consumes the chord before our hook sees it under heavy spam. Killing explorer.exe sidesteps both problems by removing the shell that owns Alt+Tab.
