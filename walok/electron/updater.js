...
function buildPhase2ApplierBat() {
  return [
    ...
    'rem --   * *-settings.json  (e.g. example-cafe-settings.json — launcher prefs)',
    ...
    'rem --   * *-data           (e.g. example-cafe-data — Electron userData, DBs)',
    'rem --   * *-assets         (e.g. example-cafe-assets — user-uploaded media)',
    ...
    'popd >NUL 2>&1',
    '',
    'rem -- Extract payload.zip. Primary path: built-in tar.exe (Win10 1803+).',
    'rem -- Fallback path: PowerShell Expand-Archive (every Win10/11 ships',
    'rem -- PowerShell 5.0+ which includes this cmdlet, so older Win10 builds',
    'rem -- or stripped kiosk images without tar.exe still apply updates).',
    'rem -- Use full paths so it works even when System32 is not in PATH',
    'rem -- (common on gaming cafe / kiosk machines running via wscript.exe).',
    'set "TAR_EXE=%SystemRoot%\\System32\\tar.exe"',
    'set "PS_EXE=%SystemRoot%\\System32\\WindowsPowerShell\\v1.0\\powershell.exe"',
    'set "EXTRACTED="',
    'if exist "%TAR_EXE%" (',
    '  echo [%date% %time%] tar -xf payload.zip (using %TAR_EXE%) >> "%APPLY_LOG%"',
    '  "%TAR_EXE%" -xf "%PENDING_DIR%\\payload.zip" -C "%INSTALL_DIR%" >> "%APPLY_LOG%" 2>&1',
    '  if not errorlevel 1 (',
    '    echo [%date% %time%] tar ok >> "%APPLY_LOG%"',
    '    set "EXTRACTED=1"',
    '  ) else (',
    '    echo [%date% %time%] tar failed exit %ERRORLEVEL% — will try PowerShell fallback >> "%APPLY_LOG%"',
    '  )',
    ') else (',
    '  echo [%date% %time%] tar.exe not found — will use PowerShell fallback >> "%APPLY_LOG%"',
    ')',
    'if not defined EXTRACTED (',
    '  if exist "%PS_EXE%" (',
    '    echo [%date% %time%] Expand-Archive payload.zip (using %PS_EXE%) >> "%APPLY_LOG%"',
    '    "%PS_EXE%" -NoProfile -ExecutionPolicy Bypass -Command "try { Expand-Archive -LiteralPath \'%PENDING_DIR%\\payload.zip\' -DestinationPath \'%INSTALL_DIR%\' -Force -ErrorAction Stop; exit 0 } catch { Write-Output $_.Exception.Message; exit 1 }" >> "%APPLY_LOG%" 2>&1',
    '    if not errorlevel 1 (',
    '      echo [%date% %time%] Expand-Archive ok >> "%APPLY_LOG%"',
    '      set "EXTRACTED=1"',
    '    ) else (',
    '      echo [%date% %time%] FAIL Expand-Archive exit %ERRORLEVEL% >> "%APPLY_LOG%"',
    '    )',
    '  ) else (',
    '    echo [%date% %time%] FAIL no extractor available (neither tar.exe nor powershell.exe found) >> "%APPLY_LOG%"',
    '  )',
    ')',
    'if not defined EXTRACTED (',
...
// basename always has a space, e.g. "EXAMPLE CAFES Server.exe"). The whitelist
...
function stageOutOfProcessApply(appRoot, pendingDir, opts) {
  ...
  const applierBase = 'example-cafe-ota-apply-' + Date.now() + '-' + process.pid
...
function applyPendingUpdateOnStartup(appRoot, _opts) {
  ...
  try {
    ...
    // than the one we're currently running (e.g. EXAMPLE CAFE.exe -> BLAST.exe), the
...
function restartApp() {
  ...
  // is the OLD exe (e.g. EXAMPLE CAFE.exe) — that's the original bug that produces a
...
function init({ appRoot, isDev, ipcMain }) {
  ...
  if (!isDev) {
    ...
    // CRITICAL handoff: when the apply step renamed the exe (EXAMPLE CAFE -> BLAST),
