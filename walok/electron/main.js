...
function createWindow(splash) {
  ...
  // Read this launcher's OTA config (channel + updateServer URL) so the
  // streaming popup handler below can fetch per-channel cookies (Netflix).
  // Mirrors the path-search logic in electron/updater.js so a launcher
  // running from any of: dev (walok/branding/), packaged (resourcesPath),
  // or fallback (appRoot) all resolve consistently.
  function loadOtaConfig() {
    const candidates = []
    if (isDev) {
      candidates.push(path.join(__dirname, '..', 'branding', 'ota-config.json'))
    } else {
      if (process.resourcesPath) candidates.push(path.join(process.resourcesPath, 'ota-config.json'))
      candidates.push(path.join(appRoot, 'ota-config.json'))
    }
  ...
  // Fetch Netflix cookies from the OTA server for this channel and convert
  // each browser-extension cookie object into the shape Electron's
  // session.cookies.set() expects. Returns [] on any failure (network,
  // 404, parse) so the popup just opens cold instead of throwing.
  async function fetchNetflixCookies() {
    const cfg = loadOtaConfig()
    if (!cfg || !cfg.channel || !cfg.updateServer) return []
    const url = cfg.updateServer.replace(/\/$/, '') + '/updates/' + encodeURIComponent(cfg.channel) + '/netflix-cookies.json'
      ...
      const req = lib.get(url, { timeout: 5000 }, (res) => {
        ...
        res.on('end', () => {
          try {
            ...
            resolve(Array.isArray(parsed.cookies) ? parsed.cookies : [])
          } catch (e) { resolve([]) }
        })
      })
  ...
  // Map a browser-extension cookie object → Electron session.cookies.set()
  // input. Required by Electron: { url, name, value }. Everything else is
  // optional but Netflix needs domain + secure + sameSite='no_restriction'
  // for the session cookie to actually be sent on the next request.
  function browserCookieToElectron(c) {
    if (!c || !c.name || c.value === undefined || c.value === null) return null
    const domain = c.domain || ''
    const cleanDomain = domain.replace(/^\./, '')
    if (!cleanDomain) return null
    // Electron requires secure=true when sameSite='no_restriction' (Chromium
    // policy: cross-site cookies must be Secure). We always set sameSite to
    // 'no_restriction' so the cookie is sent on Netflix's cross-origin XHRs
    // — therefore force secure=true regardless of what the export said.
    // For Netflix this is correct anyway (every netflix.com cookie is HTTPS).
    const out = {
      url: 'https://' + cleanDomain + (c.path || '/'),
      name: c.name,
      value: String(c.value),
      domain: domain.startsWith('.') ? domain : ('.' + cleanDomain),
      path: c.path || '/',
      secure: true,
      httpOnly: !!c.httpOnly,
      sameSite: 'no_restriction',
    }
    // Electron rejects expirationDate that's already in the past — only
    // forward future timestamps. Browser exports often ship -1 or 0 for
    // session cookies; omitting the field makes it a session cookie too.
  ...
  ipcMain.handle('open-streaming-popup', async (event, payload) => {
    try {
      ...
      const isNetflix = /(^|\.)netflix\.com$/i.test(parsed.hostname)
      ...
      // For Netflix only: pre-load any cookies the OTA admin has saved for
      // this channel BEFORE the popup loads netflix.com — otherwise the
      // first request fires unauthenticated and Netflix redirects to the
      // login page. Failures (no cookies / network error) are silent: the
      // popup just opens cold and the customer logs in themselves.
      let injectedCookieCount = 0
      if (isNetflix) {
        const raw = await fetchNetflixCookies()
        // Defense-in-depth: when the popup target is netflix.com, refuse to
        // inject cookies for any other domain. The cookie payload is admin-
        // controlled (so this isn't a privesc), but it stops a fat-fingered
        // browser export from leaking unrelated session cookies into this
        // ephemeral session — the partition is fresh anyway, but cookies
        // for other sites would still survive until popup close.
        const isNetflixDomain = (d) => {
          if (!d) return false
          const clean = String(d).replace(/^\./, '').toLowerCase()
          return clean === 'netflix.com' || clean.endsWith('.netflix.com')
        }
        for (const c of raw) {
          if (!isNetflixDomain(c && c.domain)) continue
          const electronCookie = browserCookieToElectron(c)
          if (!electronCookie) continue
          try {
            await ephemeralSession.cookies.set(electronCookie)
            injectedCookieCount++
          } catch (e) {
            // One bad cookie shouldn't poison the rest — log and continue.
            console.warn('[streaming] cookie set failed for', c.name, '-', e.message)
          }
        }
        if (injectedCookieCount > 0) {
          console.log('[streaming] injected ' + injectedCookieCount + ' Netflix cookie(s) into popup session')
        }
      }
